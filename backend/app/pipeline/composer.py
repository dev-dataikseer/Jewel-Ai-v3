...
def compose_prompt(db: Session, inp: ComposeInput) -> ComposedPrompt:
    ...
    try:
        body, negative, layer_debug = assemble_layers(
            master_layers,
            subject_layers,
    ...
    if inp.lighting_style:
        extras.append(f"Lighting style: {inp.lighting_style}.")
    if preset_addon:
        extras.append(preset_addon)
    if extras:
        body = f"{body} {' '.join(extras)}".strip()

    neg_parts = [negative, variant_neg]
    neg_out = ". ".join(dict.fromkeys(filter(None, neg_parts)))

    assert_no_jinja_leaks(body, "composed prompt")
    assert_no_jinja_leaks(neg_out, "negative prompt")
