...
// 2. Add closing </div> for Generated Card and Grid
const targetEnd = `              </div>
            </>
          </section>`;
...
// 3. Close the Generated Card's body and wrapper
// Before we add `ActionDock` or whatever, we close the card.
// Let's find: `              <ActionDock`
const actionDockTarget = `              <ActionDock`;
const actionDockReplace = `                    </div>
                  </div>
              <ActionDock`;
content = content.replace(actionDockTarget, actionDockReplace);

fs.writeFileSync('frontend/src/pages/StudioPage.tsx', content);
console.log("Success! Output card and divs replaced.");
