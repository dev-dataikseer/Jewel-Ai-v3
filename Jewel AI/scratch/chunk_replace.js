...
const sidebarRegex = /<main className="mx-auto max-w-\[1600px\] w-full px-4 sm:px-6 lg:px-8 py-6 flex-1">[\s\S]*?<div className="flex shrink-0 gap-1 lg:hidden">\s*<button\s*type="button"\s*className="ui-btn-secondary"\s*onClick=\{[^}]*\}\s*aria-label="Open workflows"\s*>\s*<Menu className="size-3\.5" \/>\s*Workflow\s*<\/button>\s*<button\s*type="button"\s*className="ui-btn-secondary"\s*onClick=\{[^}]*\}\s*aria-label="Open parameters"\s*>\s*<PanelRight className="size-3\.5" \/>\s*Params\s*<\/button>\s*<\/div>\s*<\/div>/;

const sidebarRep = `<main className="flex h-[calc(100vh-3.75rem)] w-full overflow-hidden bg-[var(--jewel-bg)]">
        <div className="flex h-full w-full divide-x divide-gray-200">
          {/* Left Sidebar */}
          <aside className="hidden w-[260px] shrink-0 flex-col overflow-y-auto bg-[var(--jewel-bg)] lg:flex border-r border-gray-200">
...
content = content.replace(sidebarRegex, sidebarRep);

// 2. Shell and Grid
const shellRegex = /<div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">\s*<div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">/;
const shellRep = `<div className="relative grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1">
...
content = content.replace(shellRegex, shellRep);

// 3. Input
const inputRegex = /\{\/\* Input \*\/\}\s*<div className="p-5 min-h-\[360px\] flex flex-col min-w-0">\s*<div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-3 shrink-0">\s*<span className="ui-label mb-0">Input<\/span>\s*\{activeJob && \(\s*<button\s*type="button"\s*onClick=\{[^}]*\}\s*className="text-xs font-semibold text-blue-600 hover:underline"\s*>\s*Edit inputs \/ New\s*<\/button>\s*\)\}\s*<\/div>/;
const inputRep = `{/* Input Card */}
...
content = content.replace(inputRegex, inputRep);

// 4. Input Close
const inputCloseRegex = /<\/div>\s*\{\/\* Output \*\/\}/;
const inputCloseRep = `</div>
                  </div>
                  {/* Output */}`;
content = content.replace(inputCloseRegex, inputCloseRep);

// 5. Output
const outputRegex = /\{\/\* Output \*\/\}\s*<div className="p-5 min-h-\[360px\] flex flex-col bg-slate-50\/30">\s*<div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-3">[\s\S]*?<\/div>\s*<\/div>/;
const outputRep = `{/* Generated Card */}
                  <div className="flex flex-col bg-white rounded-xl border border-gray-200 overflow-hidden min-h-[400px]">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-white shrink-0">
                      <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-0.5">
                              <button
                                ...
                                onClick={() => setActiveJobId(batchJobIds[batchJobIndex - 1])}
                                className="rounded p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 disabled:opacity-30"
                              >
                    ...
                    <div className="flex-1 bg-[var(--jewel-bg)] relative flex flex-col p-4">`;
content = content.replace(outputRegex, outputRep);

// 6. ActionDock
// Before ActionDock, original file had `</div> \n </div> \n </div>`.
// Since we removed Shell, and Output opens 2 divs, we still need 3 closures.
// So we DON'T need to replace `</div> \n </div> \n </div>`.
// BUT we DO need to add the end closures!
const endRegex = /<\/div>\s*<\/>\s*<\/section>/;
const endRep = `</div>
            </>
            </div>
          </section>`;
content = content.replace(endRegex, endRep);
