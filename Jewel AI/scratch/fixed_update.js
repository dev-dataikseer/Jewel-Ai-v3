...
const canvasShellOldStart = content.indexOf('              <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">');

if (mainStart === -1 || canvasShellOldStart === -1) {
    console.error("Could not find boundaries.");
    process.exit(1);
}

// 1. Everything from <main... to <div class="rounded-2xl... is replaced with New Sidebar + New Canvas Title Row
const oldSidebarAndTitle = content.substring(mainStart, canvasShellOldStart);

const newSidebarAndTitle = `<main className="flex h-[calc(100vh-3.75rem)] w-full overflow-hidden bg-[var(--jewel-bg)]">
        <div className="grid h-full w-full grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)_340px] divide-x divide-gray-200">
          ...
          {/* Canvas — stage zone */}
          <section className="flex h-full flex-col overflow-y-auto bg-[var(--jewel-bg)] relative min-w-0">
            ...
            <>
              `; // This leaves the `<>` right before the canvas shells
...
const canvasShellNew = `              <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1">
...
// 6. Fix ActionDock Closure
const actionDockOld = `              <ActionDock`;
const actionDockNew = `                    </div>
                  </div>
              <ActionDock`;
content = content.replace(actionDockOld, actionDockNew);


fs.writeFileSync(path, content);
console.log("Successfully replaced sidebar, title row, canvas wrappers and closed all tags.");
