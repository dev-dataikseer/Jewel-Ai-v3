...
class BudgetPart:
    __slots__ = ("text", "priority", "key", "source")

    def __init__(self, text: str, priority: str, key: str = "", source: str = "master"):
        self.text = text
...
def assemble_layers(
    ...
    *,
    ...
    body_parts, negative_out, debug, structured = assemble_layer_parts(
        master_layers,
        subject_layers,
        subject_layers_by_type=subject_layers_by_type,
    ...
    debug = {**debug, "structured_parts": [{"key": p.key, "priority": p.priority, "source": p.source} for p in structured]}
    return body, negative_out, debug


def assemble_layer_parts(
    master_layers: list[dict],
    subject_layers: list[dict],
    *,
    subject_layers_by_type: list[tuple[str, list[dict]]] | None = None,
    ...
    if composition_mode == "raw" and raw_override and raw_override.strip():
        ...
        assert_no_jinja_leaks(body, "composed body")
        part = BudgetPart(body, "critical", "raw_override", "master")
    ...
    for layer in sorted_master:
        ...
        if layer_type == "insert_point":
            ...
            for part in subject_rendered:
                budget_parts.append(BudgetPart(part, "critical", key or "subject", "subject"))
            debug_layers.append({"key": key, "type": layer_type, "parts": len(subject_rendered)})
            continue

        if layer_type == "variant_insert":
            if variant_text:
                budget_parts.append(BudgetPart(variant_text, priority, key or "variant", "variant"))
        ...
        if layer.get("content"):
            rendered = render_jinja(str(layer["content"]), variables)
            if rendered:
                budget_parts.append(BudgetPart(rendered, priority, key, "master"))
    ...
    # apply_token_budget_parts returns texts only — re-map by matching order of kept
    kept_budget: list[BudgetPart] = []
    kept_set = list(kept_parts)
    for original in budget_parts:
        if not kept_set:
            break
        if original.text == kept_set[0]:
            kept_budget.append(original)
            kept_set.pop(0)

    negative_out = ". ".join(filter(None, negative_parts))
    body_parts = [p.text for p in kept_budget if p.text]
    return body_parts, negative_out, {
        "layers": debug_layers,
        "word_count": word_count(" ".join(body_parts)),
        "mode": composition_mode,
