...
- `deploy/docker/docker-compose.yml`: PostgreSQL, Redis, API, Celery worker, and frontend.
- `deploy/docker/Dockerfile.api`: FastAPI service image.
- `deploy/docker/Dockerfile.web`: Vite build served by Nginx.
- `deploy/env/.env.production.example`: required production environment values.
