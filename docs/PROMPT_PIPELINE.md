...
└─────────────┘     └──────────────────┘     └─────────────────┘     └──────────────┘
       ...
       ▼                      ▼
  prompts/*.txt ──import──▶ PostgreSQL / SQLite
...
### 2.2 `prompts/*.txt` files (optional import seed)

Text files under the repo `prompts/` folder (e.g. `01_catalog_image.txt`) are parsed on API startup and imported into the DB when content changes. They are **not** read during job generation.
...
- If you edit `prompts/*.txt` and restart API → import may create **new DB versions** from files
